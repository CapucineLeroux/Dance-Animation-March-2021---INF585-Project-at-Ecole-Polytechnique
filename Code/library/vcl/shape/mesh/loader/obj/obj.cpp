...
mesh dae_mesh_extractor(const std::string& filename, buffer<buffer<int>>& vertex_correspondance){

    buffer<vec3> positions; read_from_file(filename+"mesh/vertices.txt", positions);
    buffer<vec2> texture_uv; read_from_file(filename+"mesh/uvs.txt", texture_uv);
    buffer<vec3> normals; read_from_file(filename+"mesh/normals.txt", normals);
    buffer<buffer_stack<int3,3>> faces; read_from_file(filename+"mesh/connectivity.txt", faces);
